...
[
  {
    ...
    "yaw":      rad,                  // 偏航角，弧度，自车坐标系
    "velocity": [vx, vy],             // 局部坐标系下的速度（自车运动补偿后）
    "global_velocity": [vx_global, vy_global]  // 全局绝对速度（大地坐标系）
  }
]
```
...
tracker_cfg.threshold       = cfg.track_threshold;          // 关联阈值：低于此代价的检测-轨迹对才允许匹配
tracker_cfg.metric          = metric;                       // 距离度量类型：DIST_3D / GIOU_3D / MAHALANOBIS
tracker_cfg.max_lost_frames = cfg.track_max_lost;           // 轨迹最大丢失帧数，超过后轨迹将被移除
tracker_cfg.dt              = cfg.track_dt;                 // 预测步长（秒），卡尔曼滤波的时间间隔
tracker_cfg.algo            = algo;                         // 匹配算法：GREEDY / HUNGARIAN
tracker_cfg.min_hits        = cfg.track_min_hits;           // 轨迹确认所需的最小命中次数（成为 confirmed 状态）
tracker_cfg.enable_ego_comp = cfg.track_ego_comp;           // 是否启用自车运动补偿（将检测转换到全局坐标系进行跟踪）
...
# 运行，批量处理
./build/tracking_demo outputs/frames resnet18 fp16 --score-thr 0.5 --output-format json --batch --no-warmup --classes 0,1,3,4 --track-metric dist_3d --track-threshold 7.0 --track-ego-comp 1
```
** 输出 **
